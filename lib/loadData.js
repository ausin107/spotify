...
export async function loadSearchMusic(query, maxResults, optionParam) {
  const res = await fetch(
    `${BaseUrl}search?part=snippet&key=${API_KEY}&videoCategoryId=10&maxResults=${maxResults}&type=video&q=${query}&videoEmbeddable=true${optionParam}`
  )
  const data = await res.json()
  return data
}
export async function loadSearchPlaylists(query, maxResults) {
  const res = await fetch(
    `${BaseUrl}search?part=snippet&key=${API_KEY}&maxResults=${maxResults}&order=viewCount&type=playlist&q=${query}`
  )
  const data = await res.json()
  return data
}
